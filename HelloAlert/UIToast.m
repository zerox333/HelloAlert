...
//    UIToastWindow *rootView = [UIToastWindow shareInstance];
    
    UIToastWindow *tmpWindow = [[UIToastWindow alloc] initWithFrame:CGRectMake(0, 20, 40, 30)];
    self.rootView = tmpWindow;
    rootView.hidden = NO;
    [tmpWindow release];
...
//    UIWindow *rootView = [[UIWindow alloc] initWithFrame:windowRect];
//    rootView.windowLevel = UIWindowLevelNormal;
...
-(void)showView:(UIView*)smallView WithGravity:(CGFloat)gravity Length:(TOAST_LENGTH)length{
	...
	m_length = length;
	//获取窗口句柄
	rootView = [[UIToastWindow alloc] initWithFrame:CGRectMake(0, 20, 40, 30)];
	CGRect rect = rootView.screen.bounds;
